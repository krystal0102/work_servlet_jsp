...
public class BoardDao {
	...
	public Board select (Integer no) throws ClassNotFoundException, SQLException {
		...
		// 처음에 Cursor가 BOF(Begin Of File)을 가리키고 있기 때문에 next() 메소드를 한번 호출해야 원하는 no에 해당하는 글을 가져올 수 있다
		rs.next();		
		Board board = new Board();
		board.setContent(rs.getString("content"));
		board.setNo(rs.getInt("no"));
	...
	public void insert(Board board) throws ClassNotFoundException, SQLException {
		...
		// CURDATE() : MySQL에서 제공하는 함수. sql실행하는 시간이 출력됨
	...
	// 글 수정하는 기능 (파라미터 board 객체에 저장된 no값은 수정하고자 하는 글 번호)
